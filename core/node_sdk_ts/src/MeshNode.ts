...
// SDK so the round-trip vitest exercises the same contract Python nodes use.
export class MeshNode {
  ...
  private async dispatch(env: Envelope): Promise<void> {
    ...
    const mode = this.surfaces.find((s) => s.name === surfaceName)?.invocation_mode ?? 'request_response'
