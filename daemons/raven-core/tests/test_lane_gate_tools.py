...
report wins, PR OPENED upgrades), smoke-section extraction across the
heading shapes gate reports actually use, and the spoken lines for every
path — including the named misses (no gate report, no smoke section).
...
def test_prefix_literals_match_the_kickoff_contract():
    ...
    # shell/src/utils/laneGate.ts, and here.
...
def test_unparseable_spawned_ts_never_passes_the_gate_guard(tmp_path, monkeypatch):
    ...
    assert gate == {"report": None, "pr": False}
