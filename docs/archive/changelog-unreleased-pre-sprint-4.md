# Changelog [Unreleased] (archived) — pre-Sprint 4

Entries that accumulated in [Unreleased] before Sprint 4 began. Active
[Unreleased] entries remain in CHANGELOG.md at the repository root.

These entries were never cut to a release version. They span Sprint 1
through Sprint 3 (roughly v0.5.0 through v0.8.0 territory). A future
governance lane may retroactively cut them into release headers; for
now they are preserved verbatim here.

Archived in: PR #80 (chore: archive DECISIONS.md and CHANGELOG.md
for CHOKE FILE relief). Update PR number after creation.
