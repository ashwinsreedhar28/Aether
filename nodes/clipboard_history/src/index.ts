...
import { ClipboardPoller } from './poller'
import { ClipboardStore } from './storage'

const NODE_ID = 'clipboard_history'
...
function makeRecentHandler(store: ClipboardStore) {
  return async (env: Envelope): Promise<Record<string, unknown>> => {
    ...
    if (since !== undefined) {
      ...
      if (Number.isNaN(t)) {
        throw new MeshDeny('clipboard_history_bad_since', { since })
      }
    }
    const entries = store.recent({ limit, since })
    return { entries }
  }
}

async function main(): Promise<void> {
  const secret = process.env.MESH_CLIPBOARD_HISTORY_SECRET
  if (!secret) {
    process.stderr.write(
      `[${NODE_ID}] MESH_CLIPBOARD_HISTORY_SECRET is required; refusing to start.\n`,
    )
    process.exit(2)
  }
  ...
  const nodeDir = join(dataDir, 'clipboard_history')
  mkdirSync(nodeDir, { recursive: true })
  const dbPath = join(nodeDir, 'clipboard.db')
  const markerPath = join(nodeDir, 'running')

  const store = new ClipboardStore(dbPath)
  ...
  const poller = new ClipboardPoller({ store, log })
  poller.start()
  log(`poller started — 500ms cadence`)
